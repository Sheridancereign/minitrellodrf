...
def test_assign_task_success(task, user, another_user):
...
def test_assign_task_logs_assignee_change(task, user, another_user):
    BoardMembership.objects.create(
        board=task.board,
        user=another_user,
        role=BoardMembership.Role.MEMBER,
...
def test_assign_task_denies_non_member_assignee(task, user, another_user):
    with pytest.raises(TaskDomainException) as exc_info:
        task_service.assign_task(task=task, assignee=another_user, actor=user)
